...
public:
    ...
    array(const vector_type& vec, const Allocator& alloc = Allocator())
        : m_data(mc::make_shared<impl_type>(vec, alloc))
    {
    }
    ...
    array(vector_type&& vec, const Allocator& alloc = Allocator())
        : m_data(mc::make_shared<impl_type>(std::move(vec), alloc))
    {
    }

    /**
     * @brief 特殊构造函数：当 T 是 variant 时，从 variant_reference 初始化列表构造
     * @param init variant_reference 初始化列表
     * @param alloc 分配器
     * @note 仅当 T 为 variant 时启用，避免与其他 initializer_list 构造函数产生歧义
     */
    template <typename U = T, std::enable_if_t<std::is_same_v<U, variant>, int> = 0>
    ...
    array& operator=(std::initializer_list<T> ilist) {
        m_data = mc::make_shared<impl_type>(ilist);
        return *this;
    }
...
private:
    /**
     * @brief 共享的数据指针
     * @note mutable 允许在 const 函数中修改（用于延迟初始化）
     */
    mutable mc::shared_ptr<impl_type> m_data;
