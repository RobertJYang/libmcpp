...
// 对字符串类型的特化
template <typename T>
struct signature_helper<T, std::enable_if_t<std::is_same_v<T, mc::string> || std::is_same_v<T, mc::string_view> ||
...
// 公共接口函数
template <typename T>
mc::string_view get_signature()
{
    static mc::string sig_str = detail::get_signature<T>();
    return sig_str;
}
