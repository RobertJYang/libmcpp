...
MC_API void journal_init(container_journal& j) noexcept;

// ---- begin / end ----
//
// begin：先填所有 payload 字段（relaxed store），最后 release store op → 非 none
// 调用者必须持有容器锁；调用前需把涉及节点置为 pending
MC_API void journal_begin(container_journal& j, container_op op, std::uint64_t target, std::uint64_t anchor,
                          std::uint64_t saved_a, std::uint64_t saved_b, std::uint64_t extra,
                          std::uint32_t misc) noexcept;

// end：release store op → none；不清 payload（减少写放大，读者先看 op）
MC_API void journal_end(container_journal& j) noexcept;

// ---- 读取 ----
//
// acquire load op；若 op != none 一并把 payload 打到 view 返回 true
// 否则返回 false（journal 空闲）。仅持锁者 / recover 路径调用
MC_API bool journal_load(const container_journal& j, container_journal_view& out) noexcept;

// 仅读 op（不消耗 payload 字段）
MC_API container_op journal_load_op(const container_journal& j) noexcept;
...
// 被误判为已完成
class MC_API journal_window {
public:
    journal_window(container_journal& j, container_op op, std::uint64_t target, std::uint64_t anchor,
                   std::uint64_t saved_a, std::uint64_t saved_b, std::uint64_t extra, std::uint32_t misc) noexcept;
