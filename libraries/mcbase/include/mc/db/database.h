...
public:
     ...
     */
    template <typename T>
    mc::shared_ptr<T> find(mc::string_view table_name, const query_builder& builder)
    {
     ...
     */
    template <typename T>
    std::vector<mc::shared_ptr<T>> query(mc::string_view table_name, const query_builder& builder, int limit = 0)
    {
    ...
    template <typename T>
    std::vector<mc::shared_ptr<T>> all(mc::string_view table_name)
    {
     ...
     */
    template <typename T>
    bool query(mc::string_view table_name, const query_builder& builder, std::function<bool(T&)>&& handler)
    {
    ...
    bool remove(mc::string_view table_name, const query_builder& builder, transaction* txn = nullptr);

    bool update(mc::string_view table_name, const query_builder& builder, const mc::dict& values,
                transaction* txn = nullptr);
    bool update(mc::string_view table_name, const query_builder& builder, const std::map<mc::string, variant>& values,
                transaction* txn = nullptr);
