...
inline mc::string make_full_path(mc::string_view path, mc::string_view name)
{
    if (path.empty()) {
        return mc::string(name);
    } else if (name.empty()) {
        return mc::string(path);
    } else {
        return mc::strings::concat(path, ".", name);
    }
}
...
struct type_info {
    ...
    mc::string               type_name; // 类型名，不包含模块名前缀
    reflection_metadata_wptr metadata;
    metadata_creator         creator;
};

struct factory_info {
    factory_info(factory_id_type id, mc::string name, factory_ptr f)
    ...
    mc::string      module_name; // 工厂在本模块中的名称，不包含当前模块名前缀
 ...
 */
struct module_node {
    using submodules_map = std::unordered_map<mc::string_view, std::unique_ptr<module_node>>;
    using types_map      = std::unordered_map<mc::string_view, type_info*>;

    module_node(mc::string_view name) : name(name)
    {}
    ...
    reflection_metadata_ptr get_metadata(mc::string_view type_name) const;
    reflection_metadata_ptr get_metadata(split_iterator& type_it) const;

    void collect_module_paths(mc::string& path, std::vector<mc::string>& paths) const;
    void collect_sub_factory_module_paths(mc::string& path, std::vector<mc::string>& paths) const;

    void collect_module_types(mc::string& path, std::vector<mc::string>& types) const;

    const module_node* find_module_node(split_iterator& type_it) const;
    factory_ptr        find_factory(split_iterator& type_it) const;

    module_node* new_module_node(mc::string_view name);

    mc::string            name;                 // 当前模块名称
