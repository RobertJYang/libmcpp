...
class reflection_factory::impl {
    using type_info_map    = std::unordered_map<mc::string_view, std::unique_ptr<type_info>>;
    using type_info_id_map = std::unordered_map<local_type_id_type, type_info*>;
    using factories_map    = std::unordered_map<mc::string_view, std::unique_ptr<factory_info>>;
...
public:
    ...
    std::pair<factory_id_type, factory_ptr> register_factory(mc::string_view module_name, factory_id_type factory_id,
                                                             factory_ptr factory);
    factory_id_type                         unregister_factory(mc::string_view factory_name);
    ...
    reflection_metadata_ptr get_metadata_by_name(mc::string_view type_name, const data_t& data);
    reflection_metadata_ptr get_local_metadata_by_name(mc::string_view type_name, const data_t& data);

    factory_ptr get_factory_by_name(mc::string_view module_name, const data_t& data);
    factory_ptr get_factory_by_id(factory_id_type factory_id, const data_t& data);
    void        collect_factory_names(mc::string_view path, std::vector<mc::string>& names, const data_t& data) const;

    void get_registered_types(const data_t& data, std::vector<mc::string>& types) const;

    const module_node* find_module_node(mc::string_view path, const data_t& data) const;
    void               collect_module_paths(std::vector<mc::string>& paths, const data_t& data) const;

    const mc::string& get_pretty_name() const;
    const mc::string& get_namespace_type_name() const;
};
