 ...
 */
class ModuleManagerTest : public mc::test::TestWithRuntime {
protected:
    void SetUp() override {
...
std::string dynamic_module_search_path() {
    ...
    // 直接使用 tests/module 作为搜索路径
    // 模块加载器会自动将 mc.test.dynamic 转换为 mc/test/dynamic.dylib
    return (build_root / "module" / (std::string("?") + std::string(shared_lib_ext()))).string();
}
...
TEST_F(ModuleManagerTest, TestConcurrentRequireAndUnload) {
    ...
    // 等待所有工作线程都准备好
    auto start = std::chrono::steady_clock::now();
    while (workers_ready.load(std::memory_order_acquire) != 2) {
        if (std::chrono::steady_clock::now() - start > std::chrono::milliseconds(3000)) {
