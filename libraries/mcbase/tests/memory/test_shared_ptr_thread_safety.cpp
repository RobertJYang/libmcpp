...
TEST_F(SharedPtrThreadSafetyTest, TryAddRefReleaseRefRaceCondition)
{
    ...
    for (int iter = 0; iter < iterations; ++iter) {
        ...
        threads.start_threads(num_threads - 1, [&]() {
            ...
            // 简单的等待：等待其他线程和释放线程都准备好
            while (ready_count.load(std::memory_order_acquire) < num_threads) {
                std::this_thread::yield();
            }

            // 等待释放完成
            for (int i = 0; i < 1000 && !released.load(std::memory_order_acquire); ++i) {
                std::this_thread::yield();
            }
        ...
        threads.add_thread([&]() {
            // 等待所有升级线程都准备好
            while (ready_count.load(std::memory_order_acquire) < num_threads - 1) {
                std::this_thread::yield();
            }

            ready_count.fetch_add(1, std::memory_order_release); // 通知释放线程也准备好了
...
TEST_F(SharedPtrThreadSafetyTest, DestroyedMarkerAtomicity)
{
    ...
    for (int iter = 0; iter < iterations; ++iter) {
        ...
        threads.add_thread([&]() {
            // 等待所有升级线程都准备好
            while (ready_count.load(std::memory_order_acquire) < 4) {
                std::this_thread::yield();
            }
        ...
        threads.start_threads(4, [&]() {
            ...
            while (!destroy_completed.load(std::memory_order_acquire)) {
