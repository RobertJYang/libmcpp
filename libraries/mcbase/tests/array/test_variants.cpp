...
TEST(variants_test, EraseEmptyArray)
{
    ...
    // 按标准容器语义，越界位置应抛出 out_of_range，空区间删除应为 no-op
    EXPECT_THROW(arr.erase(0), mc::out_of_range_exception);
    EXPECT_THROW(arr.erase(0, 1), mc::out_of_range_exception);
    EXPECT_THROW(arr.erase(arr.begin()), mc::out_of_range_exception);
    EXPECT_NO_THROW(arr.erase(arr.begin(), arr.end()));
}
