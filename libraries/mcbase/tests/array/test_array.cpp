...
// 测试默认构造后拷贝构造函数的共享语义
TEST(array_test, copy_constructor_from_default_constructed) {
    mc::array<int> arr1;  // 默认构造，m_data 为 nullptr
    mc::array<int> arr2 = arr1;  // 拷贝构造，应该共享数据

    // 在拷贝时，arr1.ensure_data() 会被调用，创建共享数据块
    // arr2 应该与 arr1 共享同一个数据块
    ...
    EXPECT_EQ(arr1[0], 100);
    EXPECT_EQ(arr2.size(), 1);  // arr2 也应该看到变化
    EXPECT_EQ(arr2[0], 100);
...
// 测试默认构造后拷贝赋值运算符的共享语义
TEST(array_test, copy_assignment_from_default_constructed) {
    mc::array<int> arr1;  // 默认构造，m_data 为 nullptr
    mc::array<int> arr2;
    arr2 = arr1;  // 拷贝赋值，应该共享数据

    // 在赋值时，arr1.ensure_data() 会被调用，创建共享数据块
    // arr2 应该与 arr1 共享同一个数据块
    ...
    arr1.push_back(300);
    EXPECT_EQ(arr1.size(), 1);
    EXPECT_EQ(arr1[0], 300);
    EXPECT_EQ(arr2.size(), 1);  // arr2 也应该看到变化
    EXPECT_EQ(arr2[0], 300);
}

// 测试多个默认构造对象之间的拷贝共享
TEST(array_test, multiple_copies_from_default_constructed) {
    mc::array<int> arr1;  // 默认构造
    mc::array<int> arr2 = arr1;  // 第一次拷贝
    mc::array<int> arr3 = arr1;  // 第二次拷贝
    ...
    // 验证所有对象都看到修改
    EXPECT_EQ(arr1[0], 999);
    EXPECT_EQ(arr2[0], 999);
    EXPECT_EQ(arr3[0], 999);
}

// 测试默认构造对象的链式拷贝
TEST(array_test, chained_copy_from_default_constructed) {
    mc::array<int> arr1;  // 默认构造
    mc::array<int> arr2 = arr1;
    mc::array<int> arr3 = arr2;
    mc::array<int> arr4 = arr3;
...
// 测试默认构造对象拷贝后再赋值
TEST(array_test, default_constructed_copy_then_reassign) {
    mc::array<int> arr1;  // 默认构造
    mc::array<int> arr2 = arr1;  // 共享空数据块
    ...
    EXPECT_EQ(arr1.size(), 2);
    EXPECT_EQ(arr2.size(), 2);  // arr2 也应该看到

    // 创建一个新的有数据的数组
    mc::array<int> arr3 = {100, 200, 300};

    // 将 arr3 赋值给 arr1
    arr1 = arr3;

    // 现在 arr1 和 arr3 应该共享数据
    // arr2 应该不再与 arr1 共享
    EXPECT_EQ(arr1.size(), 3);
    EXPECT_EQ(arr3.size(), 3);
    EXPECT_EQ(arr2.size(), 2);  // arr2 保持原来的数据

    EXPECT_EQ(arr1[0], 100);
    EXPECT_EQ(arr3[0], 100);

    // 修改 arr1 应该影响 arr3，但不影响 arr2
    arr1[0] = 999;
    EXPECT_EQ(arr1[0], 999);
    EXPECT_EQ(arr3[0], 999);
    EXPECT_EQ(arr2[0], 1);  // arr2 不受影响
