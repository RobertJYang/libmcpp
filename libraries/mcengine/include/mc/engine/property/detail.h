...
// 用于存储函数调用相关的数据
struct func_data {
    mc::small_function<mc::variant(mc::string_view, mc::dict&), 64> evaluator;
    mc::small_function<mc::dict(mc::string_view, mc::dict&), 64>    relate_property_collector;
    mc::dict                                                        params;
};
...
struct property_extension_data {
    mc::small_function<mc::variant(), 64>                 getter;
    mc::small_function<void(const mc::variant&), 64>      setter;
    mc::small_function<mc::variant(), 64>                 outsider_getter; // 外部getter方法
    mc::small_function<bool(const mc::variant&), 64>      outsider_setter; // 外部setter方法，返回false表示不进行实际设置
