...
public:
    using service_resolver = mc::small_function<mc::shared_ptr<service>(mc::string_view), 64>;
    using property_listener_callback = mc::small_function<void(), 64>;
    using variant_getter = mc::small_function<mc::variant(), 64>;
    using variant_setter = mc::small_function<void(const mc::variant&), 64>;

    property_helper()          = default;
    virtual ~property_helper() = default;

    static MC_API void              set_service_resolver(service_resolver resolver);
    static MC_API service_resolver& get_service_resolver();

    // 查找相关对象的辅助方法
    abstract_object* find_related_object(mc::string_view object_name);

    // 执行函数调用并返回结果
    mc::variant call_function_with_result(const detail::func_data* function_data);

    // 获取相关属性值
    MC_API mc::variant get_relate_property(const mc::engine::relate_property& relate_property);

    // 设置相关属性值
    MC_API void set_relate_property(const mc::engine::relate_property& relate_property, const mc::variant& value);

    // 连接属性变化监听器
    void connect_property_listener(abstract_object& target_object, mc::string_view property_name,
    ...
    virtual void               set_getter_function(variant_getter getter)            = 0;
    virtual void               set_setter_function(variant_setter setter) = 0;
