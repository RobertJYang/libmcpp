...
public:
    ...
    // 设置外部getter和setter方法
    void make_outsider_getter_setter(mc::small_function<value_type(), 64> outsider_getter,
                                     mc::small_function<bool(param_type), 64> outsider_setter)
    {
        ensure_extension_data();
        // 包装getter函数，将返回值转换为variant
        if (outsider_getter) {
            m_extension_data->outsider_getter = typename property_helper::variant_getter(
                [outsider_getter = std::move(outsider_getter)]() mutable -> mc::variant {
                return mc::variant(outsider_getter());
            });
        }
        // 包装setter函数，将variant参数转换为具体类型
        if (outsider_setter) {
            m_extension_data->outsider_setter = mc::small_function<bool(const mc::variant&), 64>(
                [outsider_setter = std::move(outsider_setter)](const mc::variant& value) mutable -> bool {
                T concrete_value;
...
public:
    ...
    void set_getter_function(typename property_helper::variant_getter getter) override
    {
        ensure_extension_data();
        m_extension_data->getter = std::move(getter);
    }

    void set_setter_function(typename property_helper::variant_setter setter) override
    {
        ensure_extension_data();
        m_extension_data->setter = std::move(setter);
    }
