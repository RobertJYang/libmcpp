...
namespace mc::engine {
using slot_type = mc::small_function<mc::variant(const mc::variants&), 64>;
...
struct signal_type_info : public mc::reflect::member_info_base {
    ...
    {
        return connect(&obj, std::move(slot));
    }
};
...
struct signal_info : public signal_info_base<Class> {
    ...
    {
        auto shared_slot = std::make_shared<slot_type>(std::move(slot));
        return (obj.*signal_ptr).connect([shared_slot = std::move(shared_slot)](Args... args) mutable {
            if constexpr (std::is_void_v<RetType>) {
                (*shared_slot)(mc::variants{args...});
            } else {
                return (*shared_slot)(mc::variants{args...}).as<RetType>();
            }
        });
    }
