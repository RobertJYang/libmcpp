...
#define MC_INTERFACE(name)                                                                                             \
    MC_REFLECTABLE(name);                                                                                              \
    static constexpr mc::string_view interface_name = name;                                                            \
...
#define MC_OBJECT(OBJECT_TYPE, CLASS_NAME, PATH_PATTERN, ...)                                                          \
    ...
    static constexpr mc::string_view path_pattern = PATH_PATTERN;                                                      \
    static constexpr mc::string_view class_name   = CLASS_NAME;                                                        \
    using DeclaredInterfaces = decltype(std::tuple_cat(MC_OBJECT_INTERFACE(__VA_ARGS__) std::tuple<>()));              \
    template <typename Members>                                                                                        \
    static constexpr bool check_members(const Members& members)                                                        \
    {                                                                                                                  \
        constexpr auto interfaces = std::tuple_cat(MC_OBJECT_INTERFACE(__VA_ARGS__) std::tuple<>());                   \
