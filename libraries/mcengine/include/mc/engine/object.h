...
public:
    ...
    void set(mc::string_view interface, mc::string_view property, const ValueType& value)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        m_map[std::string(interface)][std::string(property)] = value;
    }

    void unset(mc::string_view interface, mc::string_view property)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
    ...
    ValueType get(mc::string_view interface, mc::string_view property)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        auto                        interface_it = m_map.find(std::string(interface));
        if (interface_it == m_map.end()) {
    ...
    using visit_interface_callback = mc::small_function<void(mc::string_view, const ValueType&), 64>;

    void visit_interface(mc::string_view interface, const visit_interface_callback& cb)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        auto                        interface_it = m_map.find(std::string(interface));
        if (interface_it == m_map.end()) {
...
public:
    ...
    abstract_interface* get_interface(mc::string_view interface_name) const noexcept override;
    bool                has_interface(mc::string_view interface_name) const override;

    mc::connection_type connect(mc::string_view signal_name, slot_type slot,
                                mc::string_view interface_name = {}) override;
    ...
    invoke_result       invoke(mc::string_view method_name, const mc::variants& args = {},
                               mc::string_view interface_name = {}) override;
    result<mc::variant> async_invoke(mc::string_view method_name, const mc::variants& args = {},
                                     mc::string_view interface_name = {}) override;
...
protected:
    ...
    bool handle_override(property_base* prop, mc::string_view property_name, const mc::variant& value,
...
public:
    ...
    mc::string_view get_class_name() const noexcept override
    {
        return object_type::class_name;
    }

    mc::string_view get_path_pattern() const noexcept override
    {
    ...
    {
        static const detail::abstract_object_caster_registrar<object_type> _caster_registrar{};
        (void)_caster_registrar;
