...
metadata_list::metadata_list(mc::string_view name, const struct_metadata** obj_metadata, size_t count)
    : class_name(name)
{
    array.reserve(count);
...
mc::string_view metadata_list::get_class_name() const
{
    return class_name;
}
...
const struct_metadata_list& metadata_list::get_struct_metadata() const
{
    return array;
}
...
object_metadata::object_metadata(mc::string_view class_name, const struct_metadata& metadata,
...
const interface_type_info* object_metadata::get_interface_info(mc::string_view name) const
{
...
const interface_item<property_type_info> object_metadata::get_property_info(mc::string_view name,
...
const interface_item<method_type_info> object_metadata::get_method_info(mc::string_view name,
                                                                        mc::string_view interface_name) const
{
    if (interface_name.empty()) {
        auto it = m_impl->m_members.find(name);
...
const interface_item<signal_type_info> object_metadata::get_signal_info(mc::string_view name,
                                                                        mc::string_view interface_name) const
{
    if (interface_name.empty()) {
        auto it = m_impl->m_members.find(name);
...
interface_type_info_list object_metadata::get_interfaces() const
{
    interface_type_info_list interfaces;
    visit_interfaces([&](const interface_metadata& interface) {
