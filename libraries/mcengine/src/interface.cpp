...
bool interface_impl::set_property(mc::string_view property_name, const mc::variant& value)
{
    try {
...
const signal_type_info* interface_impl::get_signal_info(mc::string_view signal_name) const
{
    return get_metadata().get_signal_info(signal_name);
}

const method_type_info* interface_impl::get_method_info(mc::string_view method_name) const
{
    return get_metadata().get_method_info(method_name);
}

const property_type_info* interface_impl::get_property_info(mc::string_view property_name) const
{
...
mc::connection_type interface_impl::connect(mc::string_view signal_name, slot_type slot)
{
...
mc::variant interface_impl::emit(mc::string_view signal_name, const mc::variants& args)
{
    auto* signal_info = get_signal_info(signal_name);
    if (!signal_info) {
        return {};
...
invoke_result interface_impl::invoke(mc::string_view method_name, const mc::variants& args)
{
...
async_result interface_impl::async_invoke(mc::string_view method_name, const mc::variants& args)
{
    auto* method = get_method_info(method_name);
    if (method == nullptr) {
        return {};
...
mc::variant interface_impl::get_property(mc::string_view property_name, int options) const
{
