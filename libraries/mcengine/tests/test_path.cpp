...
TEST_F(path_test, constructor)
{
    ...
    // 使用字符串构造路径
    path p1("/engine/domain/object");
    EXPECT_EQ("/engine/domain/object", p1.str());
...
{
    path p1("/engine/domain/object");
    path p2("/engine/domain/object");
    path p3("/alpha/example");
...
TEST_F(path_test, path_joining)
{
    ...
    // 从根路径连接
    path p2 = root / "engine";
    EXPECT_EQ("/engine", p2.str());

    // 连接多个元素
    path p3 = p1 / "domain" / "object";
    EXPECT_EQ("/org/domain/object", p3.str());
...
TEST_F(path_test, parent_and_basename)
{
    ...
    EXPECT_EQ("/engine/domain", parent.str());
...
TEST_F(path_test, validation)
{
    ...
    EXPECT_TRUE(path::is_valid("/engine/domain"));
    EXPECT_TRUE(path::is_valid("/engine/domain/object"));
    EXPECT_TRUE(path::is_valid("/engine/domain/object_Test"));
    EXPECT_TRUE(path::is_valid("/engine/domain/object123"));

    // 无效路径
    EXPECT_FALSE(path::is_valid(""));
    EXPECT_FALSE(path::is_valid("org"));
    EXPECT_FALSE(path::is_valid("/org/"));
    EXPECT_FALSE(path::is_valid("/engine//domain"));
    EXPECT_FALSE(path::is_valid("/engine/domain$"));
    EXPECT_FALSE(path::is_valid("/engine/domain/"));

    // 有效元素
    EXPECT_TRUE(path::is_valid_element("org"));
    EXPECT_TRUE(path::is_valid_element("domain"));
    EXPECT_TRUE(path::is_valid_element("object"));
    EXPECT_TRUE(path::is_valid_element("object_Test"));
    EXPECT_TRUE(path::is_valid_element("object123"));

    // 无效元素
    EXPECT_FALSE(path::is_valid_element(""));
    EXPECT_FALSE(path::is_valid_element("org/"));
    EXPECT_FALSE(path::is_valid_element("org$"));
    EXPECT_FALSE(path::is_valid_element("engine domain"));
}