...
public:
    MC_OBJECT(test_object, "TestObject", "/org/test/object", (test_interface_1)(test_interface_2))
...
TEST_F(engine_test, test_object_property_changed_sig)
{
    ...
    EXPECT_EQ(path, "/org/test/object");
...
TEST(ServiceApiValidation, InitAllowsOpaqueServiceName)
{
    ::mc::engine::service service("invalid bus name");
    EXPECT_TRUE(service.init());
    EXPECT_EQ(service.name(), "invalid bus name");
    service.cleanup();
}
