...
protected:
    void SetUp() override
    {
        ...
        engine.register_const_error(mc::engine::errors::internal_error);
        engine.register_const_error(mc::engine::errors::not_supported);
        engine.register_const_error(mc::engine::errors::invalid_args);
        engine.register_const_error(mc::engine::errors::unknown_method);
        engine.register_const_error(mc::engine::errors::unknown_object);
        engine.register_const_error(mc::engine::errors::unknown_interface);
        engine.register_const_error(mc::engine::errors::unknown_property);
        engine.register_const_error(mc::engine::errors::property_read_only);
    }
...
TEST_F(ErrorEngineTest, GetFormatArgs)
{
    ...
    // 提取参数
    bool result = mc::get_error_format_args(format, args);
    ...
    // 测试无参数的格式
    mc::dict args2;
    result = mc::get_error_format_args("没有参数的格式", args2);
    EXPECT_TRUE(result);
    EXPECT_TRUE(args2.empty());

    // 测试格式错误的情况
    mc::dict args3;
    result = mc::get_error_format_args("错误的格式：${name", args3);
    EXPECT_FALSE(result);
}
...
TEST_F(ErrorEngineTest, MakeError)
{
    ...
    // 创建错误
    auto error = mc::make_error(name, format);
...
TEST_F(ErrorEngineTest, EngineStdErrorsUseMccoreNativeNames)
{
    auto& engine = error_engine::get_instance();

    auto unknown_method_info = engine.get_error_info(mc::engine::errors::unknown_method.name);
    auto unknown_iface_info  = engine.get_error_info(mc::engine::errors::unknown_interface.name);
    auto unknown_prop_info   = engine.get_error_info(mc::engine::errors::unknown_property.name);
    auto not_supported_info  = engine.get_error_info(mc::engine::errors::not_supported.name);
    auto invalid_args_info   = engine.get_error_info(mc::engine::errors::invalid_args.name);

    EXPECT_EQ(unknown_method_info.name, "mc.engine.unknown_method");
    EXPECT_EQ(unknown_iface_info.name, "mc.engine.unknown_interface");
    EXPECT_EQ(unknown_prop_info.name, "mc.engine.unknown_property");
    EXPECT_EQ(not_supported_info.name, "mc.engine.not_supported");
    EXPECT_EQ(invalid_args_info.name, "mc.engine.invalid_args");
}
...
TEST_F(ErrorEngineTest, LastError)
{
    ...
    auto err = mc::make_error(name, format);
...
TEST_F(ErrorEngineTest, ValidateErrorName)
{
    ...
    EXPECT_TRUE(mc::is_valid_error_name("x.y.z"));

    // 测试无效的错误名称
    EXPECT_FALSE(mc::is_valid_error_name(""));
    EXPECT_FALSE(mc::is_valid_error_name("invalid"));
    EXPECT_FALSE(mc::is_valid_error_name("invalid."));
    EXPECT_FALSE(mc::is_valid_error_name(".invalid"));
    EXPECT_FALSE(mc::is_valid_error_name("invalid..name"));
}
...
TEST_F(ErrorEngineTest, ReportErrorWithUnsetLastError)
{
    ...
    EXPECT_TRUE(reported->is_set());
    EXPECT_EQ(reported->get_name(), "test.unset.error");
    EXPECT_EQ(reported->get_message(), "未设置错误: test");
}
