...
TEST_F(result_test, test_cancellation_result_then)
{
    ...
    ASSERT_TRUE(chain_result.is_cancelled());
}
...
TEST_F(result_test, test_default_error_selection)
{
    ...
    EXPECT_FALSE(default_error->get_name().empty());
    EXPECT_FALSE(default_error->get_message().empty());
...
TEST_F(result_test, test_throw_and_make_method_call_exception)
{
    ...
    try {
        ...
        EXPECT_FALSE(std::string(ex.name()).empty());
    }
