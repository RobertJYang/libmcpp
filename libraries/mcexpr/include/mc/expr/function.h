 ...
 * @brief 表达式函数接口
 */
class function : public mc::shared_base {
public:
     ...
     * @brief 获取函数名称
     */
    virtual const mc::string& get_name() const = 0;
...
using function_ptr = mc::shared_ptr<function>;
...
public:
    using function_type = mc::small_function<RetType(Args...), 64>;
    ...
    simple_function(mc::string name, function_type func) : m_name(std::move(name)), m_func(std::move(func))
    {}
...
function_ptr make_simple_function(mc::string name, Func&& func)
{
    ...
    using function_type = typename simple_function_type::function_type;
    return mc::make_shared<simple_function_type>(std::move(name), function_type(std::forward<Func>(func)));
}
