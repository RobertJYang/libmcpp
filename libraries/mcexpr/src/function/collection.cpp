...
[[maybe_unused]] bool g_install_service_resolver = []() {
    mc::engine::property_helper::set_service_resolver([](mc::string_view position) {
        return func_collection::get_instance().get_service(position);
    });
    return true;
}();
...
void func_collection::add(mc::string_view position, mc::shared_ptr<mc::engine::service> service, mc::dict& functions)
{
...
mc::variant func_collection::get(mc::string_view position, mc::string_view func_name)
{
...
mc::shared_ptr<mc::engine::service> func_collection::get_service(mc::string_view position)
{
    std::lock_guard lock(m_mutex);
    std::string     position_str(position);
...
mc::dict func_collection::get(mc::string_view position)
{
    if (m_functions.find(position) == m_functions.end()) {
        return mc::dict();
    }
...
mc::dict func_collection::remove(mc::string_view position)
{
    if (m_functions.find(position) == m_functions.end()) {
        return mc::dict();
    }
...
bool func_collection::contains(mc::string_view position)
{
