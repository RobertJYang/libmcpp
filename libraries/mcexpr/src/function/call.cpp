...
mc::string generate_relate_property_key(const mc::variant& prop_value)
{
    mc::string object_name    = prop_value["object_name"].as<mc::string>();
    mc::string property_name  = prop_value["property_name"].as<mc::string>();
    mc::string interface_name = prop_value["interface"].as<mc::string>();
...
// 处理函数调用
mc::variant handle_function_call(const mc::variant& call_value, mc::string_view position)
{
    ...
    if (func_obj.is_null()) {
        elog("Function not found: ${func_name}", ("func_name", call_value["func"].as<mc::string>()));
        return mc::variant();
    }
...
// 处理属性引用
mc::variant handle_relate_property(const mc::variant& prop_value, mc::string_view position)
{
    ...
    if (service == nullptr) {
        elog("Service not found: ${object_name}", ("object_name", prop_value["object_name"].as<mc::string>()));
        return mc::variant();
    }

    // 构造对象名称字符串
    mc::string object_name = prop_value["object_name"].as<mc::string>() + "_" + mc::string(position);
    ...
    return device.get_property(prop_value["property_name"].as<mc::string>());
}

// 处理单个参数的变量注册
void handle_parameter_registration(mc::expr::context& ctx, mc::string_view key, const mc::variant& param_value,
...
mc::variant func::call(mc::string_view position, mc::dict& params)
{
    ...
    for (auto& item : m_args) {
        mc::string key = item.key.as<mc::string>();
...
mc::dict func::get_relate_properties(mc::string_view position, mc::dict& params)
{
    ...
    for (auto& item : m_args) {
        ...
        if (it == params.end()) {
            ...
            if (is_function_call(it->value)) {
                ...
                auto func_name    = it->value["func"].as<mc::string>();
