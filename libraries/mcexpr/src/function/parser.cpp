...
dotted_property_parts func_parser::parse_dotted_property(mc::string_view input)
{
    // 支持两种格式：
    ...
    if (std::regex_match(input_string, matches, pattern_with_interface)) {
        // 新语法：AAA[bmc.dev.xxx].DeviceName
        return {mc::string(matches[1].str()), mc::string(matches[3].str())}; // 返回对象名和属性名
    } else {
        // 传统语法：AAA.DeviceName
        std::regex traditional_pattern(R"(([^.]+)\.([^.]+))");
        if (!std::regex_match(input_string, matches, traditional_pattern)) {
            MC_THROW(mc::invalid_arg_exception, "Invalid property format: ${input}", ("input", input));
        }
        return {mc::string(matches[1].str()), mc::string(matches[2].str())}; // 返回对象名和属性名
    }
}

// 通用属性解析函数
relate_property func_parser::parse_property_with_type(mc::string_view input, const property_type_config& config)
{
    // 验证前缀格式
    ...
    prop.full_name     = mc::string(input);
    prop.interface     = interface_name;

    return prop;
}

relate_property func_parser::parse_property(mc::string_view input)
{
    // 解析接口信息
    std::string interface_name;
    std::regex  pattern_with_interface(R"(([^.\[]+)\[([^\]]+)\]\.([^.]+))");
    ...
    prop.object_name   = obj_name;
    prop.property_name = prop_name;
    prop.full_name     = mc::string(input); // 设置完整名称
    prop.interface     = interface_name;
    return prop;
}

relate_property func_parser::parse_sync_property(mc::string_view input)
{
    static const property_type_config config = {"<=/", "sync", "同步"};
    return parse_property_with_type(input, config);
}

relate_property func_parser::parse_ref_property(mc::string_view input)
{
    static const property_type_config config = {"#/", "ref", "引用"};
    return parse_property_with_type(input, config);
}

relate_object func_parser::parse_ref_object(mc::string_view input)
{
    // 验证格式 #/ObjectName
    ...
    // 提取对象名，去除前缀 #/
    mc::string object_name = input.substr(2);

    // 验证对象名不为空且只包含字母、数字和下划线
    if (object_name.empty() || !std::regex_match(std::string(object_name), std::regex(R"([A-Za-z_][A-Za-z0-9_]*)"))) {
    ...
    obj.full_name   = mc::string(input);

    return obj;
}

mc::string func_parser::parse_function_name(mc::string_view input)
{
    std::string input_string(input);
    // 找到第一个 $Func_ 符号
    size_t start = input_string.find("$Func_");
    if (start == std::string::npos) {
        MC_THROW(mc::invalid_op_exception, "Invalid function name format: ${input}", ("input", input));
    }
    size_t end = start + 6;
    while (end < input_string.length() && (std::isalnum(input_string[end]) || input_string[end] == '_')) {
        ++end;
    }
    if (end <= start + 6) {
        MC_THROW(mc::invalid_op_exception, "Invalid function name format: ${input}", ("input", input));
    }
    mc::string func_name = input.substr(start + 1, end - start - 1); // 包含Func_前缀
    if (!std::regex_match(std::string(func_name), std::regex(R"(Func_[A-Za-z_][A-Za-z0-9_]*)"))) {
        MC_THROW(mc::invalid_op_exception, "Invalid function name format: ${func_name}", ("func_name", func_name));
    }
    return func_name;
}

mc::string func_parser::parse_params_string(mc::string_view input)
{
    std::string input_string(input);
    // 找到第一个 { 和最后一个 }
    size_t start = input_string.find('{');
    size_t end   = input_string.rfind('}');
...
func_call func_parser::parse_function_call(mc::string_view input)
{
    func_call result;
    result.func         = parse_function_name(input);
    mc::string params_str = parse_params_string(input);
    ...
    while (pos < params_str.length()) {
        ...
        mc::string param_name = params_str.substr(name_start, pos - name_start);
        ...
        } else if (std::regex_match(std::string(param_value),
