...
#include "node/include/template_string_node.h"
...
mc::string template_string_node::to_string() const
{
    mc::string result = "\"" + m_text_parts[0];
...
{
    return mc::make_shared<template_string_node>(std::move(text_parts), std::move(expressions));
}
