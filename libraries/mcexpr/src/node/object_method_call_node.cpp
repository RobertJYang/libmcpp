...
#include "node/include/object_method_call_node.h"
...
mc::string object_method_call_node::to_string() const
{
    mc::string result = m_object->to_string() + "." + m_method_name + "(";
...
{
    return mc::make_shared<object_method_call_node>(std::move(object), std::move(method_name), std::move(args));
}
