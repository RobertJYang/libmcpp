...
parser::parser(mc::array<token> tokens) : m_tokens(std::move(tokens)), m_current(0)
{}
...
{
    // 1. 获取标识符的名称
    mc::string identifier = previous().lexeme;
...
node_ptr parser::parse_property_access(node_ptr object)
{
    ...
    while (match({token_type::dot})) {
        ...
        mc::string property = previous().lexeme;
...
node_ptr parser::parse_method_call(node_ptr object, mc::string_view method_name)
{
    // 1. 匹配左括号 '('
...
node_ptr parser::parse_template_string()
{
    ...
    mc::array<mc::string> text_parts;  // 存储普通文本
    node_ptrs             expressions; // 存储表达式节点

    // 2. 获取起始文本部分
    mc::string start_text = previous().literal.as_string();
    ...
    while (true) {
        ...
        // 4. 解析表达式内容
        mc::string expr_text = previous().lexeme;
        // 为表达式创建新的解析器
...
{
    mc::string function_name = previous().lexeme;
