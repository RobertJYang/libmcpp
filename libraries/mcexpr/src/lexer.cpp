...
lexer::lexer(mc::string_view source) : m_source(source), m_current(0), m_start(0)
{}
...
void lexer::skip_while(const char_predicate& predicate)
{
...
void lexer::skip_until(const char_predicate& predicate)
{
    while (!is_at_end() && !predicate(peek())) {
        advance();
    }
...
mc::string_view lexer::lexeme() const
{
    return m_source.substr(m_start, m_current - m_start);
}

// 提取词素（从m_start到指定位置）
mc::string_view lexer::lexeme(std::size_t end) const
{
    return m_source.substr(m_start, end - m_start);
}

// 提取词素（指定范围）
mc::string_view lexer::lexeme(std::size_t start, std::size_t end) const
{
...
// 检查是否是模板字符串
bool lexer::is_template_string(mc::string_view source)
{
    // 跳过字符直到遇到 $ 或结束引号
    size_t current = 0;
...
mc::array<token> lexer::scan_tokens()
{
    m_tokens.clear();
...
mc::array<token> lexer::scan_template_string_tokens()
{
    m_tokens.clear();
