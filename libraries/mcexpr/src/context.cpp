...
struct symbol_info {
    ...
    symbol_info(mc::string name, function_ptr func)
        : name(std::move(name)), type(symbol_type::function), function(std::move(func))
    {}

    symbol_info(mc::string name, mc::variant variable)
        : name(std::move(name)), type(symbol_type::variable), variable(std::move(variable))
    {}

    symbol_info(mc::string name, mc::engine::abstract_object* object)
...
bool context_base::has_variable(mc::string_view name, mc::string_view iface) const
{
    if (m_parent) {
...
bool context_base::has_function(mc::string_view name, mc::string_view iface) const
{
    if (m_parent) {
...
const mc::variant& context_base::get_variable(mc::string_view name, mc::string_view iface) const
{
    if (m_parent) {
...
mc::variant context_base::invoke(mc::string_view name, const mc::variants& args, mc::string_view iface) const
{
    if (m_parent) {
...
using symbol_name_map = std::unordered_map<std::string, symbol_info*>;
...
public:
    ...
    int register_variable(mc::string name, const mc::variant& value)
    {
    ...
    int register_function(function_ptr func)
    {
        if (!func) {
            return -1;
        }
    ...
    int register_object(mc::string name, mc::engine::abstract_object* obj)
    {
        if (!obj) {
            return -1;
        }
    ...
    symbol_info* get_symbol(mc::string_view name) const
    {
        std::lock_guard<std::mutex> lock(m_mutex);

        auto it = m_symbol_names.find(std::string(name));
...
int context::register_variable(mc::string name, const mc::variant& value)
{
    return m_impl->register_variable(std::move(name), value);
}

const mc::variant& context::get_variable(mc::string_view name, mc::string_view object) const
{
    if (!object.empty()) {
...
bool context::has_variable(mc::string_view name, mc::string_view object) const
{
    if (!object.empty()) {
        auto obj_symbol = m_impl->get_symbol(object);
        if (!obj_symbol) {
...
bool context::has_function(mc::string_view name, mc::string_view object) const
{
    if (!object.empty()) {
        auto obj_symbol = m_impl->get_symbol(object);
        if (!obj_symbol) {
...
mc::variant context::invoke(mc::string_view name, const mc::variants& args, mc::string_view object) const
{
    if (!object.empty()) {
        auto obj_symbol = m_impl->get_symbol(object);
        if (!obj_symbol) {
...
int context::register_function(function_ptr func)
{
    return m_impl->register_function(std::move(func));
}

int context::register_object(mc::string name, mc::engine::abstract_object* obj)
{
...
bool object_context::has_variable(mc::string_view name, mc::string_view iface) const
{
    return m_object->has_property(name, iface);
}

bool object_context::has_function(mc::string_view name, mc::string_view iface) const
{
    return m_object->has_method(name, iface);
}

const mc::variant& object_context::get_variable(mc::string_view name, mc::string_view iface) const
{
    m_property = m_object->get_property(name, iface);
    return m_property;
}

mc::variant object_context::invoke(mc::string_view name, const mc::variants& args, mc::string_view iface) const
{
