...
bool function_call_processor::matches(mc::string_view value_str) const
{
    return value_str.substr(0, 6) == "$Func_";
}

void function_call_processor::process(property_helper* property, mc::string_view value_str)
{
...
void function_call_processor::process_property_value(property_helper* property, mc::string_view value_str)
{
    ...
    auto expr_func   = call_func.template as<mc::expr::func>();
    auto relate_properties = expr_func.get_relate_properties(property->get_object()->get_position(), func_params);
    ...
    func_data->evaluator = [expr_func](mc::string_view position, mc::dict& params) mutable {
        return expr_func.call(position, params);
    };
    func_data->relate_property_collector = [expr_func](mc::string_view position, mc::dict& params) mutable {
        return expr_func.get_relate_properties(position, params);
    };
    ...
    for (const auto& entry : relate_properties) {
        auto value = entry.value.template as<mc::engine::relate_property>();
        if (value.type == "ref") {
