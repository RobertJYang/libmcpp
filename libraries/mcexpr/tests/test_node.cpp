...
{
    // 只有文本，没有表达式
    mc::array<mc::string> text_parts = {"Hello"};
...
{
    // 文本 + 表达式 + 文本
    mc::array<mc::string> text_parts    = {"Hello ", ", world"};
...
{
    // 多个表达式
    mc::array<mc::string> text_parts    = {"Value: ", " + ", " = "};
...
TEST_F(node_test, template_string_node_invalid_structure)
{
    ...
    mc::array<mc::string> text_parts2    = {"Hello"};
...
{
    mc::expr::function_ptr func = nullptr;
    int                    id   = ctx.register_function(func);
    EXPECT_EQ(id, -1);
}
