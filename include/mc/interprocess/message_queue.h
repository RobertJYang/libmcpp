...
struct queue_permissions {
    ...
    mode_t to_mode() const {
        mode_t mode = 0;
        if (owner_read)  mode |= 0400;
        if (owner_write) mode |= 0200;
        if (group_read)  mode |= 0040;
        if (group_write) mode |= 0020;
        if (others_read)  mode |= 0004;
        if (others_write) mode |= 0002;
        return mode;
    }
};
...
public:
     ...
     * @param config 队列配置
     */
    explicit message_queue(const queue_configuration& config);
     ...
     * @return 接收到的消息
     */
    std::vector<uint8_t> receive_with_priority(unsigned int& received_priority,
                                               int timeout_ms = -1);
     ...
     * @brief 检查队列是否有效
     */
    bool is_valid() const { return mqd_ != -1; }
     ...
     * @brief 获取队列ID
     */
    int descriptor() const { return mqd_; }
