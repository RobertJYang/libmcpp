...
public:
    ...
    /**
     * @brief 从std::chrono::milliseconds构造
     */
    constexpr milliseconds(const std::chrono::milliseconds& ms) : m_count(ms.count()) {}

    /**
     * @brief 获取最大毫秒值
     * @return milliseconds 最大毫秒值
     */
    static constexpr milliseconds maximum() { return milliseconds(0x7fffffffffffffffll); }
     ...
     * @brief 相等比较运算符
     */
    constexpr bool operator==(const milliseconds& c) const { return m_count == c.m_count; }
     ...
     * @return int64_t 秒数
     */
    constexpr int64_t to_seconds() const { return m_count / 1000; }
    
    /**
     * @brief 转换为std::chrono::milliseconds
     */
    constexpr operator std::chrono::milliseconds() const { 
        return std::chrono::milliseconds(m_count); 
    }
 ...
 */
inline constexpr milliseconds seconds(int64_t s) { return milliseconds(s * 1000); }
 ...
 * @return milliseconds 毫秒时间间隔
 */
inline constexpr milliseconds minutes(int64_t m) { return seconds(60 * m); }
 ...
 * @return milliseconds 毫秒时间间隔
 */
inline constexpr milliseconds hours(int64_t h) { return minutes(60 * h); }
 ...
 * @return milliseconds 毫秒时间间隔
 */
inline constexpr milliseconds days(int64_t d) { return hours(24 * d); }
...
public:
     ...
     */
    static constexpr time_point maximum() { return time_point(milliseconds::maximum()); }
     ...
     */
    constexpr const milliseconds& time_since_epoch() const { return m_elapsed; }

    /**
     * @brief 获取从纪元以来的秒数
     * @return uint32_t 秒数
     */
    constexpr uint32_t sec_since_epoch() const { return m_elapsed.count() / 1000; }
...
public:
     ...
     * @brief 转换为时间点
     */
    constexpr operator time_point() const { return time_point(seconds(m_utc_seconds)); }
     ...
     * @return uint32_t 秒数
     */
    constexpr uint32_t sec_since_epoch() const { return m_utc_seconds; }
