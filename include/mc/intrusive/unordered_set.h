...
template <
    ...
    typename Hasher,
    typename Equal,
    typename... HookOptions
>
class unordered_set {
private:
    using boost_unordered_set_type = boost::intrusive::unordered_set<
        T,
        boost::intrusive::hash<hash<Hasher>>,
        boost::intrusive::equal<equal<Equal>>,
        HookOptions...
    >;
...
public:
    ...
    // 迭代器
    iterator begin() { return m_unordered_set.begin(); }
    const_iterator begin() const { return m_unordered_set.begin(); }
    const_iterator cbegin() const { return m_unordered_set.cbegin(); }
    iterator end() { return m_unordered_set.end(); }
    const_iterator end() const { return m_unordered_set.end(); }
    const_iterator cend() const { return m_unordered_set.cend(); }

    // 容量
    bool empty() const { return m_unordered_set.empty(); }
    size_type size() const { return m_unordered_set.size(); }

    // 修改器
    std::pair<iterator, bool> insert(reference value) { return m_unordered_set.insert(value); }
    void erase(const_iterator pos) { m_unordered_set.erase(pos); }
    size_type erase(const_reference value) { return m_unordered_set.erase(value); }
    void clear() { m_unordered_set.clear(); }
    ...
    // 获取迭代器
    iterator iterator_to(reference value) { return m_unordered_set.iterator_to(value); }
    const_iterator iterator_to(const_reference value) const { return m_unordered_set.iterator_to(value); }
