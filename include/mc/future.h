...
#ifndef MCPP_FUTURE_HPP
#define MCPP_FUTURE_HPP
...
public:
    ...
    explicit Future(
        std::shared_ptr<future::State<T, Executor, Allocator>> state) :
        state_(state) {
    }
    ~Future() = default;
...
public:
    ...
    template<typename E,
             std::enable_if_t<
                 std::is_same_v<typename Executor::executor_type, E>, int> = 0>
