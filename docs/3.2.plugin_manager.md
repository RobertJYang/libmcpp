...
1. **插件目录管理**：
   - `set_plugin_dir(dir)` 设置目录
   - `plugin_dir()` 获取目录
2. **插件注册/查找**：
   - `register_plugin(shared_ptr<plugin>)` 注册
   - `find_plugin(name)` 查找
3. **插件加载/卸载**：
   - `load_plugin(name)` 加载单个
   - `load_plugins(vector<string>)` 加载列表（空列表时可作为“按需加载列表”的实现约定）
   - `unload_plugin(name)` 卸载单个
   - `unload_all_plugins()` 卸载全部
4. **生命周期**：
   - `init_plugins(service_factory&)` 初始化全部已加载插件
5. **查询**：
   - `get_loaded_plugins()` 返回已加载插件名列表
