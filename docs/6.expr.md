...
4. **类型安全保证**
   - 使用 `node_type` 枚举明确区分节点类型
   - 编译时类型检查，避免运行时错误
   - 支持类型相关的优化策略

5. **组合式架构**
   - 复杂节点由简单节点组合而成（如二元运算节点包含两个子节点）
   - 支持任意深度的嵌套表达式
   - 天然支持递归求值
...
graph LR
    ...
    subgraph Terminal["叶子节点 Terminal Nodes"]
        LiteralNode[literal_node 字面值节点]
        VariableNode[variable_node 变量引用节点]
    end
    
    subgraph Operator["运算节点 Operator Nodes"]
        BinaryOpNode[binary_op_node 二元运算节点]
        UnaryOpNode[unary_op_node 一元运算节点]
        ConditionalNode[conditional_node 条件运算节点]
    end
    
    subgraph Invocation["调用节点 Invocation Nodes"]
        FuncCallNode[function_call_node 函数调用节点]
        ObjMethodCallNode[object_method_call_node 方法调用节点]
    end
    
    subgraph Access["访问节点 Access Nodes"]
        PropAccessNode[property_access_node 属性访问节点]
    end
    
    subgraph Composite["复合节点 Composite Nodes"]
        TplStrNode[template_string_node 模板字符串节点]
    end
...
classDiagram
    ...
    class template_string_node {
        ...
        -expressions: node_ptrs
        +get_text_parts() vector~string~
        +get_expressions() node_ptrs
        +evaluate(ctx) variant
    }
...
1. 对象系统集成
   - 通过 `mc::expr::engine` 管理表达式计算
   - 可直接构造 `mc::expr::engine` 或使用单例 `mc::expr::engine::get_instance()`
   - 支持对象属性的动态访问和计算
...
4. 路径解析挂载
   - 系统初始化时由 `mc::engine::engine` 安装路径解析回调：当对象路径模式中包含 `${...}` 时，自动调用表达式引擎对模板路径求值
   - 模板路径解析流程：`lexer::scan_template_string_tokens()` → `parser::parse()` → `template_string_node::evaluate()`；计算结果必须为字符串，否则抛出 `mc::invalid_arg_exception`
...
4. 文件结构
   - `mc/expr/engine.h`: 表达式引擎对外 API
   - `mc/expr/lexer.h`: 词法分析器（独立实现，支持模板字符串 `${...}`）
   - `mc/expr/parser.h`: 语法分析器（递归下降，含条件表达式/位运算/逻辑短路等）
   - `mc/expr/node.h`: AST 节点类型与工厂函数
   - `mc/expr/context.h`: 上下文与对象上下文 API
   - 其他：与对象系统/路径解析/错误处理等在 `engine/` 模块
...
2. 特殊处理
   - 处理转义字符
   - 模板字符串的分段解析（`${...}` 内支持成对大括号嵌套计数）
   - 数字进制识别：`0x` 十六进制、`0b` 二进制、`0` 八进制、含小数点的浮点
   - 布尔字面量：`true`/`false`
   - 错误处理和报告
...
2. 函数/方法/属性解析
   - 函数调用：解析函数名和参数列表
   - 方法调用：`obj.method(args...)`（`obj` 需为变量名，对象方法由对象上下文 `has_method/invoke` 解析）
   - 属性访问：`obj.prop` 与链式 `obj.prop1.prop2`；当中间值为 `variant object` 且含键时继续取值
   - 模板字符串：`template_start/template_expr/template_middle/template_end` 组成 `template_string_node`
...
```cpp
// 获取表达式引擎单例
auto& engine = mc::expr::engine::get_instance();
...
// DBus 场景：直接获取 GVariant*
GVariant* gv = engine.evaluate_as_gvariant("x + y", ctx);
```
...
4. `pow(base, exponent)`：计算幂
   - 支持负指数和零次方
   - 限制：当前实现禁止负底数（base < 0 抛出 `invalid_arg_exception`）
...
1. `concat(str1, str2, ...)`：字符串连接
   - 支持任意数量的参数
   - 自动连接任意 `variant` 参数（由 `variant` 的 `operator+=` 驱动）
...
4. `to_bool(value)`：转换为布尔值
   ...
   - 无法转换时抛出 `mc::invalid_arg_exception`
...
// 提供具名注册函数（静态库构建下匿名命名空间静态注册器会被链接器丢弃，
// 因此必须使用具名外部链接函数，由 register_builtin_modules() 统一调度调用）
...
新加内建模块时，需要在 `libraries/mcexpr/src/builtin.cpp` 的 `register_builtin_modules()` 中追加对应的
`detail::register_builtin_module_xxx()` 调用，确保启动时注册到 builtin 上下文。
...
1. 变量和方法解析
    - 方法调用：`obj.method(args...)` 中 `obj` 必须是变量标识符（上下文注册的对象名），通过对象的 `has_method()/invoke()` 解析
    - 属性访问：
      - `obj.prop` 若 `obj` 为对象名，则通过对象 `has_property()/get_property()` 获取
      - 链式 `a.b.c`：若 `a.b` 的求值结果为 `variant` 的 `object` 类型且包含键 `c`，则继续取值
    - 未匹配到时回退到父级上下文
