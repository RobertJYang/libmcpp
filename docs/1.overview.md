...
**接口规范层**：通过DDS（设备描述规范）定义部件的标准化接口契约，使用MC_INTERFACE宏声明接口元数据，MC_REFLECT宏实现属性反射，确保编译时类型安全和运行时元数据可访问性。

**数据规范层**：通过CSR（配置状态寄存器）建立统一的配置状态数据模型，采用mc::dict键值对结构存储，支持嵌套层次和动态扩展，实现配置数据的序列化、持久化和跨进程共享。
...
部件对象是系统中硬件设备的软件抽象表示，采用**DDS（设备描述规范，Device Description Standard）+ CSR（配置状态寄存器，Configuration & Status Register）**双组件架构实现统一管理。
...
flowchart TB
    subgraph Component["部件对象 (Device Component)"]
        ...
        subgraph DDS["DDS - 设备描述规范<br/>Device Description Standard"]
            direction TB
        ...
        subgraph CSR["CSR - 配置状态寄存器<br/>Configuration & Status Register"]
            direction TB
            CSRDesc["<b>职责</b>：存储部件的运行时状态"]
