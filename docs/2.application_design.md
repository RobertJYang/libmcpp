...
### 4.0 Plugin、Service、Supervisor关系图

以下图表展示了Plugin、Service和Supervisor三者之间的核心关系：

```mermaid
graph TD
    %% 定义主要组件
    A[Application] --> PM[PluginManager]
    A --> SF[ServiceFactory]
    A --> SM[ServiceManager]
    A --> SV[SupervisorManager]
    ...
    %% 插件注册服务类型
    P1 -->|注册| SCT1[Service类型1]
    P1 -->|注册| SCT2[Service类型2]
    P2 -->|注册| SCT3[Service类型3]
    ...
    %% 服务实例化
    SF -->|创建实例| SI1[Service实例1]
    SF -->|创建实例| SI2[Service实例2]
    SF -->|创建实例| SI3[Service实例3]
    ...
    %% 监督管理器
    SV -->|创建管理| SU1[Root Supervisor]
    SU1 -->|创建管理| SU2[Supervisor 1]
    SU1 -->|创建管理| SU3[Supervisor 2]
    ...
    %% 样式定义
    classDef app fill:#FF69B4,stroke:#333,stroke-width:2px,color:white;
    classDef manager fill:#FFA07A,stroke:#333,stroke-width:2px,color:white;
    classDef plugin fill:#6495ED,stroke:#333,stroke-width:2px,color:white;
    classDef serviceType fill:#98FB98,stroke:#333,stroke-width:2px;
    classDef serviceInstance fill:#F0E68C,stroke:#333,stroke-width:2px;
    classDef supervisor fill:#DDA0DD,stroke:#333,stroke-width:2px,color:white;
    
    %% 应用样式
    class A app;
    class PM,SF,SM,SV manager;
    class P1,P2 plugin;
    class SCT1,SCT2,SCT3 serviceType;
    class SI1,SI2,SI3 serviceInstance;
    class SU1,SU2,SU3 supervisor;
```

主要交互职责：

1. **Plugin与Service的关系**：
   - Plugin负责注册Service类型到ServiceFactory
   - Plugin可以注册多个Service类型
   - Plugin不直接管理Service实例

2. **Service与Supervisor的关系**：
   - Service实例由ServiceFactory创建
   - Service实例受Supervisor监督管理
   - Service实例可以声明对其他Service的依赖

3. **Supervisor层次结构**：
   - Supervisor组织为树状结构
   - 不同Supervisor负责不同的Service集合
   - Supervisor负责服务生命周期和错误恢复

#### 4.0.1 应用场景：Hi1822网卡驱动管理

下图展示了一个具体应用场景：加载Hi1822网卡驱动插件后，创建网卡管理服务，由Supervisor监控服务运行状态。
...
sequenceDiagram
    ...
    participant NIC as Hi1822Service
    
    %% 插件加载阶段
    App->>App: 加载Hi1822 CSR
    App->>PM: 加载Hi1822Driver插件
    PM->>SF: 注册Hi1822Service类型
    
    %% 服务创建阶段
    App->>SF: 请求创建Hi1822Service实例
    SF->>SM: 创建Hi1822Service("nic1")
    SM->>NIC: 初始化(配置参数)
...
这个场景展示了:
- **插件加载**: Hi1822Driver插件被加载并注册网卡服务类型
- **服务创建**: 通过ServiceFactory创建特定网卡("nic1")的管理服务
- **启动流程**: 服务初始化并启动网卡
- **监控机制**: Supervisor持续监控网卡服务的健康状态、资源使用
- **故障恢复**: 当检测到网卡服务故障时，Supervisor会尝试重启服务
- **关闭流程**: 应用关闭时，有序停止服务
