...
Application 类通过组合模式集成了多个管理器，每个管理器负责特定领域的功能：
...
classDiagram
    ...
    class ServiceFactory {
        +register_service_type() bool
        +create_service() service_ptr
    }
    
    class ServiceManager {
        +create_service() service_ptr
        +destroy_service() void
        +get_service() service_ptr
    }
    
    class ConfigManager {
        +load_config() bool
        +get_service_config() dict
        +validate_config() bool
    }
    
    class SupervisorManager {
        +create_supervisor() supervisor_ptr
        +destroy_supervisor() void
        +get_supervisor() supervisor_ptr
    }
