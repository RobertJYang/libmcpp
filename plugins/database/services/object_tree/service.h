...
public:
    ...
    // 更新对象属性
    error_code update_object_property(const path& object_path, const char* property_name, const void* value, size_t value_size);
    ...
    static void register_options(po::options_description& cli_opts, po::options_description& cfg_opts) {
        cfg_opts.add_options()
            ("object_tree.max_objects", po::value<size_t>()->default_value(10000), 
             "对象树服务的最大对象数量")
            ("object_tree.max_depth", po::value<size_t>()->default_value(10), 
             "对象树的最大深度");
    }
