...
public:
    ...
    void register_options(po::options_description& cmdline_opts,
        ...
        cmdline_opts.add_options()
            ("example.log-level", po::value<std::string>()->default_value("info"),
             "日志级别 (debug, info, warn, error)")
            ("example.output-file", po::value<std::string>(),
             "输出文件路径");
        config_file_opts.add_options()
            ("example.enable-feature", po::value<bool>()->default_value(false),
             "是否启用特定功能");
    }
};
...
extern "C" appbase::plugin* create_plugin() {
    return new example::example_plugin();
}