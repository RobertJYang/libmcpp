...
void query_database(const std::string& sql) {
    ...
    if (sql.find("SELECT") != 0 && sql.find("INSERT") != 0 && sql.find("UPDATE") != 0 && sql.find("DELETE") != 0) {
...
// 模拟综合操作
void process_user_request(const std::string& username, const std::string& password, const std::string& operation, const std::string& server) {
    try {
        // 尝试认证用户
