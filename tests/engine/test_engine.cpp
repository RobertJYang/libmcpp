...
MC_REFLECT(test_engine::test_interface_1, (m_i32, "i32")(m_str, "str")(m_vec, "vec")(m_normal_v, "normal_v"),
           (rewite_method)(rewite_method, "not_rewite_method")(add_values))
MC_REFLECT(test_engine::test_interface_2, (m_variant, "variant"))
MC_REFLECT(test_engine::test_object, (m_iface_1, "iface_1")(m_iface_2, "iface_2"), (rewite_method))
...
TEST_F(engine_test, test_object_property_changed_sig)
{
    ...
    EXPECT_EQ(path, sformat("/org/test/object_{}_{}", obj->get_object_id(), obj->m_iface_1.m_i32.get_value() + 100));
...
TEST_F(engine_test, test_property_changed_sig)
{
    ...
    // 指定订阅接口1的i32属性
    mc::dict values;
    obj->m_iface_1.m_i32.property_changed().connect([&](const mc::variant& value, const auto& prop) {
        values[prop.get_name()] = value;
    });
...
TEST_F(engine_test, test_property_changed_sig_use_abstract_object)
{
    ...
    // 2：通过路径全局对象表里面找到对象
    auto result = table.find<mc::engine::by_path>(obj->get_object_path());
    ASSERT_FALSE(result.is_end());
    auto* res_obj = const_cast<mc::engine::abstract_object*>(&result.get());
...
TEST_F(engine_test, test_property_changed_event_visible_to_parent_filter)
{
...
TEST_F(engine_test, test_property_changed_event_owns_value_payload)
{
    ...
    mc::variant emitted_value(42);

    obj->property_changed().connect([&](const mc::variant& value, const auto&) {
        last_value = value;
    });
...
TEST_F(engine_test, test_managed_object)
{
    ...
    EXPECT_EQ(get_managed_objects(*test_obj), objects({"/org/test/o1", "/org/test/o2", "/org/test/o3"}));
    ...
    EXPECT_EQ(get_managed_objects(*obj2), (mc::variants{"/org/test/o2/middle/a", "/org/test/o2/middle/b"}));

    // 然后再添加 middle 对象，这样 obj2_mid_a、obj2_mid_b 都会被移动到 middle 对象下
    auto obj2_mid = create_object("/org/test/o2/middle");
    EXPECT_EQ(get_managed_objects(*obj2_mid), (mc::variants{"/org/test/o2/middle/a", "/org/test/o2/middle/b"}));
    ...
    EXPECT_EQ(get_managed_objects(*obj2_mid), (mc::variants{"/org/test/o2/middle/a", "/org/test/o2/middle/b"}));
    EXPECT_EQ(get_managed_objects(*obj2), (mc::variants{"/org/test/o2/middle", "/org/test/o2/middles"}));
    ...
    EXPECT_EQ(get_managed_objects(*test_obj_new),
              objects({"/bmc/dev/Accessor/Accessor_1v8_0101010301", "/bmc/dev/Accessor/Accessor_0v8_0101010301",
                       "/bmc/dev/Accessor/Accessor_1v2_0101010301"}));
}
...
TEST_F(engine_test, test_managed_object_comprehensive)
{
    ...
    EXPECT_EQ(get_managed_objects(*deep_ab), objects({"/org/test/deep/a/b/c", "/org/test/deep/a/b/d"}));

    // 继续添加中间节点
    auto deep_a_obj = create_object("/org/test/deep/a");
    EXPECT_EQ(get_managed_objects(*deep_a_obj), objects({"/org/test/deep/a/b"}));
    EXPECT_EQ(get_managed_objects(*test_obj), objects({"/org/test/o1", "/org/test/o2", "/org/test/deep/a"}));

    // 最后添加顶层节点
    auto deep_obj = create_object("/org/test/deep");
    EXPECT_EQ(get_managed_objects(*deep_obj), objects({"/org/test/deep/a"}));
    EXPECT_EQ(get_managed_objects(*test_obj), objects({"/org/test/o1", "/org/test/o2", "/org/test/deep"}));
    ...
    EXPECT_EQ(get_managed_objects(*root_obj), objects({"/org/test", "/org/unrelated", "/org/sub/a"}));
    ...
    // 验证子对象被重新分配给根节点
    EXPECT_EQ(get_managed_objects(*root_obj), objects({"/org/test", "/org/unrelated", "/org/sub/a/b", "/org/sub/a/c"}));
    ...
    EXPECT_EQ(get_managed_objects(*test_obj),
              objects({"/org/test/o1", "/org/test/o2", "/org/test/deep", "/org/test/o11", "/org/test/special_dash",
                       "/org/test/special_dot", "/org/test/special_underscore"}));

    // ======= 极端情况测试 =======
    // 测试8：超长路径
    std::string long_path = "/org/test/very/very/very/very/very/very/very/very/very/very/very/very/long/path";
    ...
    EXPECT_EQ(get_managed_objects(*rebuild_top), objects({"/org/rebuild/a", "/org/rebuild/b", "/org/rebuild/c"}));
    ...
    EXPECT_EQ(get_managed_objects(*rebuild_a), objects({"/org/rebuild/a/1", "/org/rebuild/a/2", "/org/rebuild/a/mid"}));
...
TEST_F(engine_test, ServiceTimeoutCalls)
{
    ...
    auto result = service.timeout_call(::mc::milliseconds(100), service.name(), obj->get_object_path(),
                                       obj->m_iface_1.get_interface_name(), "add_values", "ii", args);
    EXPECT_EQ(result.as_int32(), 10);

    // shm_timeout_call 在超时或服务不存在时会抛出异常，需要捕获
    try {
        auto opt = service.shm_timeout_call(::mc::milliseconds(10), "org.openubmc.undefined", obj->get_object_path(),
                                            obj->m_iface_1.get_interface_name(), "add_values", "ii", args);
...
TEST_F(engine_test, test_object_rewite_method)
{
    ...
    // 对象重载了该方法
    EXPECT_EQ(obj_ptr->invoke("rewite_method", {"111"}, "org.test.test_interface_1"), "test_object:rewite_method: 111");
