...
- **测试用例总数**：约 200+ 个测试用例（已优化，删除了重复和无法运行的测试用例）。
- **一致性检查**：已完成针对重复测试体与无断言测试的自动化扫描，已删除重复的测试用例；唯一的 `DISABLED_PerformanceBenchmark` 用例仅用于性能占位。
...
- `test_property*.cpp` 系列：测试属性读写、同步/引用属性、外部访问者、并发修复以及属性处理器。
  - `test_property_processors.cpp`：测试属性处理器（已优化，合并了基础测试和 Details 测试，删除了无法运行的 GTEST_SKIP 测试用例）。
...
- **注意事项**：
  - 避免取消注释 `DISABLED_PerformanceBenchmark` 性能用例前请补充断言与运行时监控
  - 若新增注册/反射相关测试，需同步维护 `mc::engine::register_*` 相关初始化逻辑
  - 对于需要完整环境的测试用例，应提供可注入的 mock 环境，而不是使用 `GTEST_SKIP` 跳过
