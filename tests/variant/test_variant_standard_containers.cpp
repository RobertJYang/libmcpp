...
TEST_F(VariantStdContainersTest, MapConversion) {
    ...
    std::map<std::string, std::vector<int>> complex_result = v.as<std::map<std::string, std::vector<int>>>();
...
TEST_F(VariantStdContainersTest, UnorderedSetConversion) {
    ...
    for (int value : int_set) {
        ASSERT_TRUE(result2.find(value) != result2.end()) << "使用 as 方法转换的集合缺少值: " << value;
    }
}
...
TEST_F(VariantStdContainersTest, MixedContainerConversion) {
    ...
    for (const auto& [key, value] : complex_container) {
        ...
        for (size_t i = 0; i < value.size(); ++i) {
            ASSERT_EQ(result[key][i].first, value[i].first) << "转换回的对组 first 不匹配，键: " << key << ", 位置: " << i;
            ASSERT_EQ(result[key][i].second, value[i].second) << "转换回的对组 second 不匹配，键: " << key << ", 位置: " << i;
        }
    }
}
