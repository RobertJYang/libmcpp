...
TEST_F(VariantDeepCopyTest, EmptyContainersDeepCopy) {
    ...
    EXPECT_EQ(v1.get_object().data(), nullptr);
    EXPECT_EQ(v2.get_object().data(), nullptr);

    // 空 array - 优化：空 array 保持延迟构造，内部数据是 nullptr
    variants empty_arr;
...
/**
 * @brief 测试强类型数组（variant_extension）的深拷贝
 *
 * mc::array<int> 作为 variant_extension 存储时，array_impl 只实现了 copy()
 * 默认的 deep_copy() 会委托给 copy()，实现浅拷贝语义：
 * - 创建新的 array_impl 对象（结构独立）
 * - 但不递归深拷贝元素（对基本类型是值拷贝，对引用类型会共享）
 */
TEST_F(VariantDeepCopyTest, TypedArrayExtensionDeepCopy) {
    // 创建强类型 int 数组，存储为 variant_extension
    mc::array<int> typed_arr = {10, 20, 30};
    variant        v1(typed_arr);
    ...
    // 验证数据内容相同
    auto ext1_ptr = v1.as_extension();
    auto ext2_ptr = v2.as_extension();
    ASSERT_NE(ext1_ptr, nullptr);
    ASSERT_NE(ext2_ptr, nullptr);

    // 获取类型信息
    auto type1 = ext1_ptr->get_type_name();
    auto type2 = ext2_ptr->get_type_name();

    // mc::array<int> 对应的是 array_impl<int>
    using array_impl_type = mc::detail::array_impl<int>;
    auto* arr1_ptr        = dynamic_cast<array_impl_type*>(ext1_ptr.get());
    auto* arr2_ptr        = dynamic_cast<array_impl_type*>(ext2_ptr.get());
    ASSERT_NE(arr1_ptr, nullptr) << "Failed to cast to array_impl<int>, type: " << type1;
    ASSERT_NE(arr2_ptr, nullptr) << "Failed to cast to array_impl<int>, type: " << type2;
    EXPECT_EQ(arr1_ptr->size(), 3);
    EXPECT_EQ(arr2_ptr->size(), 3);
    EXPECT_EQ((*arr1_ptr)[0], 10);
    EXPECT_EQ((*arr2_ptr)[0], 10);

    // 关键验证：extension 对象本身是不同的（copy() 创建了新对象）
    EXPECT_NE(arr1_ptr, arr2_ptr);

    // 验证内部数据也是独立的（array_impl 的拷贝构造会拷贝内部 vector）
    EXPECT_NE(arr1_ptr->data(), arr2_ptr->data());

    // 验证结构独立性：修改拷贝的数组结构不影响原数组
    arr2_ptr->push_back(40);
    EXPECT_EQ(arr1_ptr->size(), 3);
    EXPECT_EQ(arr2_ptr->size(), 4);
    EXPECT_EQ((*arr2_ptr)[3], 40);

    // 验证元素独立性：对于基本类型（int），修改元素值也不影响原数组
    (*arr2_ptr)[0] = 100;
    EXPECT_EQ((*arr1_ptr)[0], 10);
    EXPECT_EQ((*arr2_ptr)[0], 100);
}

/**
 * @brief 测试包含强类型数组的嵌套结构深拷贝
 *
 * 验证在复杂嵌套结构中，强类型数组的深拷贝行为正确
 */
TEST_F(VariantDeepCopyTest, NestedTypedArrayDeepCopy) {
    // 创建包含强类型数组的 dict
    mc::array<int> typed_arr = {100, 200, 300};
    dict           d1({{"typed_array", variant(typed_arr)}, {"value", 999}});
    variant        v1(d1);
    ...
    // 验证拷贝成功
    EXPECT_EQ(v2["value"], 999);
    EXPECT_TRUE(v2["typed_array"].is_extension());

    // 获取拷贝的强类型数组
    auto ext1_ptr = v1["typed_array"].as_extension();
    auto ext2_ptr = v2["typed_array"].as_extension();
    ASSERT_NE(ext1_ptr, nullptr);
    ASSERT_NE(ext2_ptr, nullptr);
    ...
    auto* arr1_ptr        = dynamic_cast<array_impl_type*>(ext1_ptr.get());
    auto* arr2_ptr        = dynamic_cast<array_impl_type*>(ext2_ptr.get());
    ASSERT_NE(arr1_ptr, nullptr);
    ASSERT_NE(arr2_ptr, nullptr);
    ...
    // 验证数据相同
    EXPECT_EQ((*arr1_ptr)[0], 100);
    EXPECT_EQ((*arr2_ptr)[0], 100);

    // 验证独立性：修改不相互影响
    (*arr2_ptr)[0] = 111;
    arr2_ptr->push_back(400);

    EXPECT_EQ((*arr1_ptr)[0], 100);
    EXPECT_EQ((*arr2_ptr)[0], 111);
    EXPECT_EQ(arr1_ptr->size(), 3);
    EXPECT_EQ(arr2_ptr->size(), 4);

    // 验证外层 dict 也独立
    v2["value"] = 888;
    EXPECT_EQ(v1["value"].as_int32(), 999);
    EXPECT_EQ(v2["value"].as_int32(), 888);
}
