...
{
    bool called = false;
    auto conn   = test_signal.connect([&called](int value) {
        called = true;
    });
...
TEST_F(connection_manager_test, add_multiple_connections)
{
    ...
    auto conn1 = test_signal.connect([&called1](int) {
        called1 = true;
    });
    auto conn2 = test_signal.connect([&called2](int) {
        called2 = true;
    });
...
{
    bool called = false;
    auto conn   = test_signal.connect([&called](int) {
        called = true;
    });
...
TEST_F(connection_manager_test, clear)
{
    ...
    auto conn1 = signal1.connect([&called1](int) {
        called1 = true;
    });
    auto conn2 = signal2.connect([&called2](int) {
        called2 = true;
    });
...
TEST_F(connection_manager_test, multiple_signals)
{
    ...
    auto conn2 = signal2.connect([&called2](int, int) {
        called2 = true;
    });
