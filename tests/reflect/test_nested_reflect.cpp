...
public:
    ...
    test_user(const std::string& username, const test_contact& contact, test_status status,
        ...
        : m_username(username), m_contact(contact), m_status(status),
          m_permissions(permissions), m_addresses(addresses) {}
...
TEST(NestedReflectTest, NestedEnumReflection) {
    ...
    std::map<std::string, test_address> addresses = {
        {"home", test_address("北京", "中关村", 123)},
...
TEST(NestedReflectTest, ComplexNestedSerialization) {
    ...
    std::vector<test_permission> permissions = {
