...
protected:
    ...
    // 辅助方法：创建一个带有前缀的节点
    ref_ptr_type create_node_with_prefix(void* leaf, const std::string& prefix) {
...
TEST_F(NodeTest, BasicConstruction) {
    ...
    EXPECT_EQ(empty_node->m_leaf, nullptr);
    ...
    EXPECT_EQ(leaf_node->m_leaf, leaves[0]);
    ...
    EXPECT_EQ(prefix_node->m_leaf, leaves[1]);
...
TEST_F(NodeTest, GetKey) {
    ...
    EXPECT_EQ(node_a->m_leaf, leaves[0]);
    EXPECT_EQ(std::string(node_a->m_prefix.data(), node_a->m_prefix.size()), "a");
    
    EXPECT_TRUE(node_p->is_leaf());
    EXPECT_EQ(node_p->m_leaf, leaves[1]);
    EXPECT_EQ(std::string(node_p->m_prefix.data(), node_p->m_prefix.size()), "p");
    
    // 测试直接查找a节点
    key_buffer<> key_a("a");
    auto [result_a, found_a] = root->get(key_a);
    EXPECT_TRUE(found_a);
    EXPECT_EQ(result_a, leaves[0]);
    
    // 测试查找ap路径 (组合路径)
    key_buffer<> key_ap("ap");
    auto [result_ap, found_ap] = root->get(key_ap);
    EXPECT_TRUE(found_ap);
    EXPECT_EQ(result_ap, leaves[1]);
    
    // 测试查找bc路径 (组合路径)
    key_buffer<> key_bc("bc");
    auto [result_bc, found_bc] = root->get(key_bc);
    EXPECT_TRUE(found_bc);
    EXPECT_EQ(result_bc, leaves[2]);
    
    // 测试查找不存在的键
    key_buffer<> key_ac("ac");
    auto [result_ac, found_ac] = root->get(key_ac);
    EXPECT_FALSE(found_ac);
    EXPECT_EQ(result_ac, nullptr);
    
    // b不是叶子节点，所以找不到
    key_buffer<> key_b("b");
    auto [result_b, found_b] = root->get(key_b);
    EXPECT_FALSE(found_b);
    EXPECT_EQ(result_b, nullptr);
}
...
TEST_F(NodeTest, WalkTree) {
    ...
    // 测试条件中止遍历
    walk_results.clear();
    auto limited_fn = [&walk_results](const key_view& key, const void* val) -> bool {
        walk_results.push_back({std::string(key.data(), key.size()), const_cast<void*>(val)});
...
TEST_F(NodeTest, WalkPrefix) {
    ...
    auto [result_an, found_an] = root->get(key_an);
    EXPECT_TRUE(found_an);
    EXPECT_EQ(result_an, leaves[1]);
    
    walk_results.clear();
    key_buffer<> key_at("at");
    auto [result_at, found_at] = root->get(key_at);
    EXPECT_TRUE(found_at);
    EXPECT_EQ(result_at, leaves[2]);
    
    walk_results.clear();
    key_buffer<> key_bc("bc");
    auto [result_bc, found_bc] = root->get(key_bc);
    EXPECT_TRUE(found_bc);
    EXPECT_EQ(result_bc, leaves[3]);
}

// 测试降序节点
TEST_F(NodeTest, ReverseNodeOrder) {
    // 创建降序排序的节点
    using reverse_node_type = reverse_node;
