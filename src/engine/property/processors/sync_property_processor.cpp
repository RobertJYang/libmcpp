...
void sync_property_processor::hook_sync_property(property_helper* property, const mc::expr::relate_property& relate_property) {
    ...
    property->set_setter_function([property](const mc::variant& value) {
        MC_THROW(mc::invalid_op_exception, "设置同步属性值不被允许: ${name}", ("name", property->get_name()));
    });
}
...
void sync_property_processor::hook_sync_properties(property_helper* property, mc::mutable_dict& relate_properties) {
    // 断开旧连接
    property->clear_connection_slots();
