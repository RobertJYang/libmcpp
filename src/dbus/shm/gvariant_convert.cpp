...
static GVariant* new_gvariant_by_type(const variant& v) {
    switch (v.get_type()) {
    case type_id::bool_type:
        return g_variant_new_boolean(v.as_bool());
    case type_id::int8_type:
        return g_variant_new_byte(v.as_int8());
    ...
    case type_id::string_type:
        return g_variant_new_string(v.as_string().data());
    default:
        MC_THROW(mc::invalid_arg_exception, "unsupported type: ${type}",
...
std::tuple<GVariant*, const char*> gvariant_convert::to_gvariant_inner(const variant& v,
    ...
    case DBUS_TYPE_VARIANT:
        res = g_variant_new_variant(new_gvariant_by_type(v));
        break;
    default:
