...
bool plugin_manager::register_plugin(std::shared_ptr<plugin> plugin) {
    ...
    dlog("注册插件'${name}'，版本'${version}'", ("name", name)("version", info.version));
...
bool plugin_manager::load_plugins(const std::vector<std::string>& plugin_names) {
    ...
    // 否则加载目录下的所有插件
    try {
        for (const auto& entry : std::experimental::filesystem::directory_iterator(m_plugin_dir)) {
...
bool plugin_manager::load_dynamic_library(const std::string& plugin_name, void*& handle,
    ...
    if (!handle) {
        elog("加载动态库失败: ${path}, 错误: ${error}", ("path", lib_path)("error", dlerror()));
        return false;
    }
    ...
    // 创建智能指针，使用自定义删除器
    plugin = std::shared_ptr<mc::plugin>(raw_plugin, [destroy_func](mc::plugin* p) {
        destroy_func(p);
    });
