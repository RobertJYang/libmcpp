...
// 构造函数
json_config_manager::json_config_manager()
...
// 析构函数
json_config_manager::~json_config_manager() = default;

// 解析命令行参数
bool json_config_manager::parse_command_line(int argc, char** argv) {
    try {
...
// 加载配置文件
bool json_config_manager::load_config_file(const std::string& file_path) {
...
// 处理单个配置对象
void json_config_manager::process_config(const variant& config) {
    if (!config.is_dict()) {
        wlog("跳过非对象配置项");
        return;
    }
...
// 获取插件列表
std::vector<std::string> json_config_manager::get_plugin_names() const {
    // 首先检查命令行参数
...
// 获取插件目录
std::string json_config_manager::get_plugin_dir() const {
    return m_plugin_dir;
}

// 获取线程数
unsigned int json_config_manager::get_thread_count() const {
